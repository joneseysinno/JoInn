...
pub(crate) fn join_refuse_catches_drop() -> Result<bool, String> {
    let sum = sum_cell();
    ...
    let _ = state.inject("sum", 0, int_val(1)?, 0);
    let _ = state.inject("sum", 0, int_val(2)?, 0);
    Ok(matches!(state.run(), Verdict::Refused(r) if r.check == joinn_frame::CheckId::Join))
}
