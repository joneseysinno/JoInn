...
pub(crate) fn ungranted_is_refused() -> Result<bool, String> {
    let sum = sum_cell();
    let sh = hash(&sum.coding);
    let src = format!(
        "body {{ codex 1 genome {{ cell:{} as sum }} grants {{ cli missing }} wires {{ }} budget {{ steps 10 }} lineage none }}\n",
        sh.to_hex()
    );
    ...
    cells.insert(sh, sum);
    let mut state = match BodyState::new(body, cells, joinn_prim::sealed_natives(), 7) {
        Verdict::Ok(s) => s,
        Verdict::Refused(r) => return Err(r.reason),
    };
    Ok(matches!(
        state.inject("sum", 0, int_val(1)?, 0),
        Verdict::Refused(r) if r.reason.contains("ungranted")
    ))
}
