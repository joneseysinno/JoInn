...
pub(crate) fn p21_findings_control(art: &joinn_gate::Artifact) -> bool {
    ...
    match fs::read_to_string(root.join("xtask").join("src").join("main.rs")) {
        Ok(src) => src.contains("join(\"Findings\")"),
        Err(_) => true,
    }
}
