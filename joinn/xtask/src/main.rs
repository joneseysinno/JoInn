...
fn main() -> ExitCode {
    ...
    let result = match cmd.as_str() {
        ...
        "gate" => match args.next().as_deref() {
            ...
            Some("1") => fns::gate_one().and_then(fns::require_full),
            Some("2") => fns::gate_two().and_then(fns::require_full),
            Some("2.1") => fns::gate_two_one().and_then(fns::require_full),
            Some("2.2") => fns::gate_two_two().and_then(fns::require_full),
